...
4. Agora, vamos adicionar um filtro a um campo que já está sendo usado pela visualização. 
    ...
    Quando você usa o filtro em um campo usado no Visual em que o campo é agregado (por exemplo, soma, média ou contagem), está filtrando o valor *agregado* em cada ponto de dados. Portanto, pedir para filtrar o visual acima, em que **Vendas deste ano > 500.000** significaria ver apenas o ponto de dados **13 – Charleston Fashion Direct** no resultado. Filtrar em [medidas de modelo](../transform-model/desktop-measures.md) sempre se aplica ao valor agregado do ponto de dados.
