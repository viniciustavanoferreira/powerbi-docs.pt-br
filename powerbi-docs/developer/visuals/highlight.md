...
Os elementos visuais com mapeamento categórico de exibição de dados têm `capabilities.json` com o parâmetro `"supportsHighlight": true`. Por exemplo:
...
O código-fonte do visual padrão, após a remoção do código desnecessário, terá a seguinte aparência:
...
Importe as interfaces necessárias para processar dados do Power BI:
...
Crie o elemento raiz `div` para valores de categoria:
...
Limpe o conteúdo dos elementos div antes de renderizar novos dados:
...
Obtenha categorias e valores de medida do objeto `dataView`:
...
Crie elementos `div` e `p` para exibir e visualizar valores de exibição de dados no DOM do visual:
...
Aplique os estilos necessários para que os elementos usem `flex box` e defina as cores para elementos div:
...
Os visuais com mapeamento de exibição de dados de matriz têm `capabilities.json` com o parâmetro `"supportsHighlight": true`. Por exemplo:
...
|   Row1   |   Row2   |   Row3   |   Coluna1   |   Coluna2   |   Column3   |   Valores   |
|-----|-----|------|-------|-------|-------|-------|
...
Crie o projeto visual padrão e aplique o exemplo de `capabilities.json`.
...
Crie dois elementos `div` para o layout visual:
...
Verifique os dados no método `update`, para garantir que o visual obtenha os dados:
...
Crie a função `treeWalker` para percorrer a estrutura de dados da matriz:
...
Em que `matrixNode` é o nó atual, `levels` são colunas de metadados desse nível de hierarquia, `div` – elemento pai para elementos HTML filho.

A `treeWalker` é uma função recursiva, é necessário criar o elemento `div` e `p` para o texto como um cabeçalho e chamar a função para elementos filho do nó:
...
Chame a função para elementos raiz de coluna e linha da estrutura da exibição de dados de matriz:
...
Gere selectionID para os nós e crie botões para exibir os nós:
...
A etapa principal de usar o realce é processar uma matriz adicional de valores.

Se inspecionar o objeto do nó de terminal, você poderá ver que a matriz values tem duas propriedades: value e highlight:
...
* [Leia sobre mapeamentos de exibição de dados de matriz](dataview-mappings.md#matrix-data-mapping)

* [Leia sobre as funcionalidades do visual](capabilities.md)
